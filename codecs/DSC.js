...
module.exports = new Codec('DSC', function(multiplexer, input, line) {
  ...
  switch(input.values[2]) {

    case '00': // routine category
      switch (input.values[3]) {
	...
	handled = true;
        get_position = true;
	distress = true;
	switch (input.values[3]) { // Nature of Distress
  ...
  if (get_position) {
    var position = parsePosition(input.values[5])
    values.push({
