...
const debug = Debug('signalk-parser-nmea0183/PBVE')
...
/*
 ...
 @return Array
*/
function toInts(alphas: string): number[] {
...
/*
 ...
 @return Int
*/
function convertToValue(values: string): number {
...
/*
 ...
 @return Int
*/
function convertToAlarmValue(values: string): number {
  const parts = toInts(values)
...
/*
 ...
 @return Int
*/
function convertToEngineMinutes(values: string): number {
  const parts = toInts(values)
  return 16 * parts[0]! + parts[1]!
}

// Gauge value emitted for product codes 'D' (oil pressure) and 'E' (coolant
// temperature). The `meta` object is carried through from the per-product
// `schema` table so downstream clients see a fully-populated descriptor.
interface GaugeMeta {
  description: string
  units: string
  displayName: string
...
const PBVE: HookFn = function (
  ...
  const schemaTable = schema as unknown as Record<
    string,
    { path?: string; meta: GaugeMeta } | undefined
  >
  ...
  if (productCode === 'B') {
    ...
    let convertedValue: GaugeValue

    if (productCode === 'D') {
      const conditionalValue = (derivedValue: number): number =>
        gaugeUnits === 'AA' ? derivedValue * 6894.757 : derivedValue * 100000
