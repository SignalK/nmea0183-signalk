...
const debug = Debug('signalk-parser-nmea0183/GSV')
/** NMEA0183 GSV
...
interface GsvAccumulator {
  ...
  gnss: string | undefined
  antennaType?: string
}

const GSV: HookFn = function (
  input: ParserInput,
  session: ParserSession
): Delta | null {
  const { id, parts, tags, talker } = input

  let gsvData = session['gsvData'] as GsvAccumulator | undefined
  if (!gsvData) {
    gsvData = {
  ...
  gsvData.nextSentenceNumber = (gsvData.nextSentenceNumber ?? 0) + 1
