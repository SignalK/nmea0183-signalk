...
describe('GSV', () => {
  it('GPGSV converts to GPS', () => {
    ...
    let r = parser.parse(testData[0]!) as any
    expect(r).to.be.null
    r = parser.parse(testData[1]!) as any
    expect(r).to.be.null
    r = parser.parse(testData[2]!) as any
  ...
  it('GPGSV with not repeated numOfSentences, satsInView converts', () => {
    ...
    const parser = new Parser()
    let r = parser.parse(data[0]!) as any
    expect(r).to.be.null
    r = parser.parse(data[1]!) as any
    expect(r).to.be.null
    r = parser.parse(data[2]!) as any
    expect(r).to.not.be.null
    const pathValue = r.updates[0]!.values[0]
    pathValue.path.should.equal('navigation.gnss.satellitesInView')
  ...
  it('Skips an out-of-order sentence and resets state', () => {
    ...
    let r = parser.parse(testData[0]!) as any
    expect(r).to.be.null
    r = parser.parse(testData[2]!) as any // sentence 3 instead of 2
    expect(r).to.be.null
  })
