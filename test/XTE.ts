...
describe('XTE', () => {
  it('Converts OK using individual parser', () => {
    const delta = new Parser().parse('$GPXTE,A,A,0.67,L,N*6F') as any
  ...
  it("Doesn't choke on an empty sentence", () => {
    const delta = new Parser().parse('$GPXTE,,,,,*72') as any
  ...
  it('Handles R direction (steer right) and km units', () => {
    const delta = new Parser().parse('$GPXTE,A,A,0.67,R,K*74') as any
