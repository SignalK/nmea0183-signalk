...
const distressSentences: Record<string, string> = {
...
describe('DSC', () => {
  it('Position converts ok', () => {
    const delta = new Parser().parse(nmeaLinePos) as any
  ...
  it('Distress converts ok', () => {
    const delta = new Parser().parse(nmeaLineDistress) as any
  ...
  distressCases.forEach(({ code, path }) => {
    it(`Distress nature ${code} maps to ${path}`, () => {
      const delta = new Parser().parse(distressSentences[code]!) as any
  ...
  it('Unhandled sentence produces a notification without throwing', () => {
    const delta = new Parser().parse(nmeaLineUnhandled) as any
  ...
  it('Consecutive parses return independent deltas (no shared state)', () => {
    ...
    const d1 = parser.parse(nmeaLinePos) as any
    const d2 = parser.parse(nmeaLineDistress) as any
  ...
  it('Safety category (08) falls through to unhandled notification', () => {
    const delta = new Parser().parse(nmeaLineSafety) as any
    delta.updates[0]!.values.should.containItemWithProperty(
      'path',
      'notifications.dsc_parser'
    )
  ...
  it('Urgency category (10) falls through to unhandled notification', () => {
    const delta = new Parser().parse(nmeaLineUrgency) as any
    delta.updates[0]!.values.should.containItemWithProperty(
      'path',
      'notifications.dsc_parser'
    )
