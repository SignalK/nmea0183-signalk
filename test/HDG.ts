...
describe('HDG', () => {
  it('Converts OK using individual parser', () => {
    const delta = new Parser().parse('$SDHDG,181.9,,,0.6,E*32') as any
    ...
    delta.updates[0]!.values.find(
      (pv: any) => pv.path === 'navigation.magneticVariation'
    )!.value.should.be.closeTo((0.6 / 180) * Math.PI, 0.005)
  })

  it('Sentence with just heading works', () => {
    const delta = new Parser().parse('$HCHDG,51.5,,,,*73') as any
  ...
  it('Applies easterly deviation as a positive correction', () => {
    const delta = new Parser().parse('$INHDG,180,5,E,10,W*7F') as any
    delta.updates[0]!.values.find(
      (pv: any) => pv.path === 'navigation.magneticDeviation'
    )!.value.should.be.closeTo((5 / 180) * Math.PI, 0.00001)
  })

  it('Sentence with all fields converts and applies corrections', () => {
    const delta = new Parser().parse('$INHDG,180,5,W,10,W*6D') as any

    delta.updates[0]!.values.find(
      (pv: any) => pv.path === 'navigation.headingMagnetic'
    )!.value.should.be.closeTo(((180 - 5) / 180) * Math.PI, 0.00001)
    delta.updates[0]!.values.find(
      (pv: any) => pv.path === 'navigation.headingCompass'
    ...
    delta.updates[0]!.values.find(
      (pv: any) => pv.path === 'navigation.headingTrue'
    )!.value.should.be.closeTo(((180 - 5 - 10) / 180) * Math.PI, 0.00001)
  })

  it("Doesn't choke on empty sentences", () => {
    const delta = new Parser().parse('$SDHDG,,,,,*70') as any
