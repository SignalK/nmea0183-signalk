...
describe('seatalk', () => {
  ...
  ['$PSMDST,','$STALK,'].forEach(prefix => {
    ...
    it(`${prefix} 0x10 AWA converted`, () => {
      const fullSentence = utils.appendChecksum(`${prefix}${apparentWindAngleData}`)
      const delta = new Parser().parse(fullSentence)
    ...
    it(`${prefix} 0x11 AWS converted`, () => {
      const fullSentence = utils.appendChecksum(`${prefix}${apparentWindSpeedData}`)
      const delta = new Parser().parse(fullSentence)
    ...
    it(`${prefix} 0x20 STW converted`, () => {
      const fullSentence = utils.appendChecksum(`${prefix}${speedThroughWaterData}`)
      const delta = new Parser().parse(fullSentence)
    ...
    it(`${prefix} 0x26 STW converted`, () => {
      const fullSentence = utils.appendChecksum(`${prefix}${averageSpeedThroughWaterData}`)
      const delta = new Parser().parse(fullSentence)
    ...
    it(`${prefix} 0x27 Water temperature converted`, () => {
      const fullSentence = utils.appendChecksum(`${prefix}${waterTemperatureData}`)
      const delta = new Parser().parse(fullSentence)
    ...
    it(`${prefix} 0x50 Latitude and 0x51 Longitude converted`, () => {
      const fullSentence = utils.appendChecksum(`${prefix}${latitudeData}`)
      const delta = parser.parse(fullSentence)

      const fullSentence2 = utils.appendChecksum(`${prefix}${longitudeData}`)
      const delta2 = parser.parse(fullSentence2)
      ...
      delta2.updates[0].values[0].value["longitude"].should.be.closeTo(-33, 0.5)
    })
