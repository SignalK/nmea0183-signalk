...
describe('seatalk', () => {
  ;['$PSMDST,', '$PSMDST,R,', '$STALK,'].forEach((prefix: any) => {
    ...
    it(`${prefix} 0x50 Latitude and 0x51 Longitude converted`, () => {
      ...
      var delta = parser.parse(fullSentence) as any

      fullSentence = utils.appendChecksum(`${prefix}${longitudeData}`)
      delta = parser.parse(fullSentence) as any
      delta.updates[0]!.values.should.containItemWithProperty(
        'path',
        'navigation.position'
      )
    ...
    it(`${prefix} 0x56 date disabled`, () => {
      ...
      const delta = parser.parse(fullSentence) as any

      delta.updates[0]!.values.should.containItemWithProperty(
        'path',
        'navigation.datetime'
      )
      delta.updates[0]!.values[0]!.value.should.equal(
        '2024-04-04T17:08:34.000Z'
      )
    })
    ...
    it(`${prefix} 0x57 S=1 branch sets DD=0x94`, () => {
      ...
      delta.updates[0]!.values.find(
        (v: any) => v.path === 'navigation.gnss.horizontalDilution'
