...
describe('MWD', () => {
  it('speed & direction data (#1)', () => {
    const delta = new Parser().parse('$IIMWD,,,046.,M,10.1,N,05.2,M*0B') as any
  ...
  it('speed & direction data (#2)', () => {
    const delta = new Parser().parse('$IIMWD,046.,T,046.,M,10.1,N,,*17') as any
  ...
  it('speed & direction data (#3)', () => {
    const delta = new Parser().parse('$IIMWD,046.,T,,,,,5.2,M*72') as any
  ...
  it('missing direction data', () => {
    const delta = new Parser().parse('$IIMWD,,,,,,,5.2,M*3A') as any

    should.equal(delta, null)
  })

  it('missing speed data', () => {
    const delta = new Parser().parse('$IIMWD,,,046.,M,,,,*0F') as any

    should.equal(delta, null)
  })

  it('improper direction designator (#1)', () => {
    const delta = new Parser().parse('$IIMWD,,,046.,T,,,,*16') as any
  ...
  it('improper speed designator (#1)', () => {
    const delta = new Parser().parse('$IIMWD,,,046.,M,10.1,n,,*7F') as any

    should.equal(delta, null)
  })

  it('improper speed designator (#2)', () => {
    const delta = new Parser().parse('$IIMWD,,,046.,M,,,0.0,m*4C') as any
  ...
  it('improper direction designator for degrees magnetic, using degrees true', () => {
    const delta = new Parser().parse(
      '$IIMWD,046.,T,0.,m,10.1,N,5.2,M*51'
    ) as any
  ...
  it('improper speed designator for m/s, using speed in kn', () => {
    const delta = new Parser().parse('$IIMWD,,,046.,M,10.1,N,0.0,m*1C') as any
