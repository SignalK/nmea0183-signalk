...
describe('MDA', () => {
  it('check parsing with barometric pressure and wind speed in knots', () => {
    ...
    delta.updates[0].values
      .find((x) => x.path === 'environment.outside.pressure')
      .should.contain.property('value', 98500)
    delta.updates[0].values
      .find((x) => x.path === 'environment.outside.temperature')
      .should.contain.property('value', 276.25)
    delta.updates[0].values
      .find((x) => x.path === 'environment.water.temperature')
      .should.contain.property('value', 278.75)
    delta.updates[0].values
      .find((x) => x.path === 'environment.outside.humidity')
      .should.contain.property('value', 0.4)
    delta.updates[0].values
      .find((x) => x.path === 'environment.outside.humidityAbsolute')
      .should.contain.property('value', 0.03)
    delta.updates[0].values
      .find((x) => x.path === 'environment.outside.dewPointTemperature')
      .value.should.be.closeTo(276.55, 0.01)
    delta.updates[0].values
      .find((x) => x.path === 'environment.wind.directionTrue')
      .value.should.be.closeTo(1.5707, 0.0001)
    delta.updates[0].values
      .find((x) => x.path === 'environment.wind.directionMagnetic')
      .value.should.be.closeTo(1.4835, 0.0001)
    delta.updates[0].values
      .find((x) => x.path === 'environment.wind.speedOverGround')
