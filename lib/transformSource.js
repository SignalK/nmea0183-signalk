...
module.exports = function transformSource(data, sentence, talker) {
  ...
  data.delta.updates = data.delta.updates.map((update) => { // eslint-disable-line no-param-reassign
    ...
    const tagTalker = _source.split(':')[0]

    if (talker === 'nmea0183') {
      talker = 'SK' // eslint-disable-line no-param-reassign
    }

    update.source = { // eslint-disable-line no-param-reassign
